<div>
  ...
  <x-form-select wire:model='limit' name="limit" :options="$limits" />
    ...
    @endpush
  <div class="columns-2 ml-5 h-24 font-sans text-xs leading-3 text-left box-border text-zinc-800">

    <div
      id="loc-list"
      class="float-left w-1/3 h-24 overflow-auto font-sans text-xs text-left box-border text-zinc-800"
      style="height: 530px;"
    >
      <ul
      id="list"
      class="block h-full float-left clear-left p-0 m-0 text-xs box-border"
    ...
    <div class="w-2/3 h-full" wire:ignore x-ref='map' id='map' style='height: 600px;border:red solid 1px'></div>
    </div>
  </div>
  <div class="col">
                {{ $addresses->links() }}
